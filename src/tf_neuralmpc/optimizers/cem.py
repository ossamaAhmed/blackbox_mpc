"""
tf_neuralmpc/optimizers/cem.py
==============================
"""
...
class CEMOptimizer(OptimizerBase):
    ...
    def __init__(self, planning_horizon, max_iterations, population_size, num_elite, dim_U, dim_O,
        ...
        self.elites_dim = [int(self.num_agents), int(num_elite), int(self.planning_horizon), int(self.dim_U)]
    ...
    def __call__(self, current_state, time_step, exploration_noise):
       ...
       """
        ...
        def iterate(t, mean, variance):
            lower_bound_dist = mean - self.action_lower_bound_horizon
            upper_bound_dist = self.action_upper_bound_horizon - mean
            ...
            values, indices = tf.nn.top_k(rewards, k=self.num_elite, sorted=True)
            samples = tf.transpose(samples, [1, 0, 2, 3])
            elites = tf.zeros(self.elites_dim, dtype=tf.float32)
            for agent in tf.range(self.num_agents):
                elites = tf.concat([elites[:agent], tf.expand_dims(tf.gather(samples[agent], indices[agent]), 0),
                                    tf.zeros([samples.shape[0] - 1 - agent, self.num_elite, *samples.shape[2:]], dtype=tf.float32)],
                                   axis=0)
                elites.set_shape(self.elites_dim)
            new_mean = tf.reduce_mean(elites, axis=1)
            new_variance = tf.reduce_mean(tf.square(elites -
                                                    tf.tile(tf.expand_dims(new_mean, 1), [1, tf.shape(elites)[1], 1, 1])),
                                          axis=1)

            mean = self.alpha * mean + (tf.constant(1, dtype=tf.float32) - self.alpha) * new_mean
            variance = self.alpha * variance + (tf.constant(1, dtype=tf.float32) - self.alpha) * new_variance
        ...
        num_optimization_iters, mean, variance = tf.while_loop(
            ...
            loop_vars=[tf.constant(0, dtype=tf.int32), self.previous_solution,
